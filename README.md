...
- 沙箱环境没有真实支付回调或评价接口契约，订单列表/详情新增“演示推进”入口把订单状态按 1→2→3→4→5 推一格，并把评价入口指向可填写的本地演示页。叠加数据存在 Preferences 中，按登录账号 scope 隔离，UI 显著标识“本地演示”，不影响后端真实订单。
...
- 结算和支付相关页面不等同于已经完成真实订单提交、支付回调和支付结果闭环；
- 项目演示视频可随求职材料提供。
