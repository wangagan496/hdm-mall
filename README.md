...
- 演示环境没有真实支付回调与评价接口契约，支付、发货、签收、评价状态推进由本地覆盖层完成，列表、详情、支付结果页统一按「服务端状态 + 本地覆盖层」合并展示。
...
- 支付结果不做本地判定，统一以服务端订单状态（`GET /member/order/{id}`）为准；订单列表、订单详情和支付结果页共用 `OrderPresentation` 里的状态文案与倒计时口径。
- 取消订单只在旧接口不存在（HTTP 404/405）时降级到兼容路径，业务失败（如“已发货不能取消”）原样返回，不会对同一订单重复下发写请求。
- 订单状态支持本地覆盖层：演示环境没有真实支付回调或评价接口契约，支付、发货、签收、评价等推进写入按登录账号 scope 隔离的 Preferences 覆盖层；列表、详情、支付结果页统一按「服务端状态 + 本地覆盖层」合并评估，并把无法与后端匹配的本地状态显式标记出来，不把本地演示状态伪装成真实订单状态。
